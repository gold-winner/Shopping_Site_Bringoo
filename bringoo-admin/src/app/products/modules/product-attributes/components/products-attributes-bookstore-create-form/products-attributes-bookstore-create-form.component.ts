...
import { CoverTypeEnum, ProductAttributesBookstoreCreateInput } from '../../../../../../shared/api/auth/data-contracts';
...
type formType = ToFormGroupType<ProductAttributesBookstoreCreateInput>;

@UntilDestroy()
@Component({
  selector: 'app-products-attributes-bookstore-create-form',
  templateUrl: './products-attributes-bookstore-create-form.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => ProductsAttributesBookstoreCreateFormComponent),
      multi: true,
    },
    {
...
export class ProductsAttributesBookstoreCreateFormComponent implements ControlValueAccessor, AfterViewInit {
  ...
  buildForm(): void {
    this.form.valueChanges.pipe(untilDestroyed(this), distinctUntilChanged()).subscribe((value: typeof this.form.value) => {
      this.onFormChanges(value as ProductAttributesBookstoreCreateInput);
    });
  }
  ...
  private onFormChanges(value: ProductAttributesBookstoreCreateInput): void {
  ...
  set value(value: ProductAttributesBookstoreCreateInput) {
    this.form.patchValue(value);
    this.onChange(value);
    this.onTouched();
  }

  get value(): ProductAttributesBookstoreCreateInput {
    return this.form.value as ProductAttributesBookstoreCreateInput;
  }

  registerOnChange(fn: (value: ProductAttributesBookstoreCreateInput) => void): void {
    this.onChange = fn;
  }

  writeValue(value: ProductAttributesBookstoreCreateInput): void {
    if (value) {
      this.value = value;
    }
  }
