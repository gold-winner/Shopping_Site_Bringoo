...
export class SelectProductLinkFilterFormComponent extends DynamicFilterFormComponent {
  ...
  private createDefaultFilters({ category, subCategory, brand, isDeposit, storeId, storeProductRecommendationId }: filters): any[] {
    const and: any[] = [];
    if (category) {
    ...
    if (storeId) {
      and.push({ 'storeProductRecommendation.storeId': storeId });
    }
