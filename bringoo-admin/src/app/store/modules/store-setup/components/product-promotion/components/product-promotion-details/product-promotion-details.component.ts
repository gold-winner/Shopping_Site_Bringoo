...
import {
  ...
  StoreProductPromotionUpdateInput,
...
type FormType = ToFormGroupType<StoreProductPromotionUpdateInput & { id: string; storeId: string; productLinkIds: string[] }>;
...
export class ProductPromotionDetailsComponent extends DynamicForm<ProductPromotionEntity> {
  ...
  filterForItems!: Pick<ProductPromotionItemEntity, 'storeProductPromotionId'>;

  customImportParams: any;

  fields: CrudFields<ProductPromotionItemEntity> = ['id', 'productLinkId', 'storeProductPromotionId'];
  join: string[] = [
  ...
  columns: CrudColumn<ProductPromotionItemEntity>[] = [
    {
      label: '',
      isSortable: false,
      getField(item: ProductPromotionItemEntity): EntityValue {
    ...
    },
    {
      ...
      type: 'link',
      link(item: ProductPromotionItemEntity): string {
        return `./${item.id}`;
      },
    },
    {
  ...
  constructor(
    ...
    public readonly crudStoreProductPromotionService: CrudProductPromotionService,
    public readonly crudStoreProductPromotionItemService: CrudProductPromotionItemService,
  ...
  beforePatch(value: ProductPromotionEntity): ProductPromotionEntity {
    this.filterForItems = {
      storeProductPromotionId: value.id,
    };
    return value;
  }
  ...
  update(): void {
    ...
    if (id) {
      const input: StoreProductPromotionUpdateInput = this.form.value as StoreProductPromotionUpdateInput;
      this.crudStoreProductPromotionService.update(id, input).subscribe();
    }
  }
  ...
  updateImages(
    input: Pick<StoreProductPromotionUpdateInput, 'mainImageUrls' | 'headerImageUrls' | 'previewImageUrls'>,
