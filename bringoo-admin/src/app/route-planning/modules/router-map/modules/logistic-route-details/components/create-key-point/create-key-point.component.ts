...
@Component({
  ...
  templateUrl: 'create-key-point.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class CreateKeyPointComponent extends DynamicForm<KeyPointCreateInput> {
  @Input() set hideDropPointCreation(input: boolean) {
    if (input) {
      this.pointTypes.splice(this.pointTypes.indexOf(KeyPointTypeEnum.DROP_OFF), 1);
      this.form.patchValue({ pointType: KeyPointTypeEnum.PICK_UP });
    }
  }
